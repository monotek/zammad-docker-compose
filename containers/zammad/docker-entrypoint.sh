...
if [ "$1" = 'zammad-init' ]; then
  ...
  sed -e "s#.*adapter:.*#  adapter: postgresql#g" \
    -e "s#.*database:.*#  database: ${POSTGRESQL_DB}#g" \
    -e "s#.*username:.*#  username: ${POSTGRESQL_USER}#g" \
